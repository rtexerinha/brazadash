...
export function ServiceReviewForm({
  ...
  const submitMutation = useMutation({
    mutationFn: async (data: {
      ...
      const res = await apiRequest("POST", "/api/services/reviews", data);
      return res.json();
    },
    onSuccess: () => {
      toast({ title: "Review Submitted", description: "Thank you for your feedback!" });
      queryClient.invalidateQueries({ queryKey: ["/api/bookings"] });
      queryClient.invalidateQueries({ queryKey: ["/api/services/providers", providerId, "reviews"] });
      onSuccess?.();
    },
    onError: (error: Error) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={4}
              data-testid="input-service-review-comment"
            />
          </div>
              ...
              {photos.map((photo, index) => (
                <div key={index} className="relative">
                  ...
                  <button
                    ...
                    data-testid={`button-remove-service-photo-${index}`}
                  >
                    <X className="h-3 w-3" />
                  </button>
                </div>
              ...
              {photos.length < 5 && (
                <label className="h-20 w-20 flex items-center justify-center border-2 border-dashed rounded-lg cursor-pointer hover-elevate">
                  <input
                    ...
                    className="hidden"
                    data-testid="input-service-photo-upload"
                  />
          ...
          <div className="flex gap-3 justify-end">
            {onCancel && (
              <Button type="button" variant="outline" onClick={onCancel} data-testid="button-cancel-service-review">
                Cancel
              </Button>
            )}
            <Button type="submit" disabled={submitMutation.isPending} data-testid="button-submit-service-review">
